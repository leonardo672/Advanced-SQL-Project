-- =========================================
-- PostgreSQL 15+ Advanced Project Script
-- Purpose: Demonstrate advanced SQL skills
-- Features: CTEs, nested queries, window functions, joins, aggregates
-- =========================================

-- 1.Create Database if not exists
-- In PostgreSQL, you need to connect to 'postgres' first
-- This block checks and creates database conditionally
DO $$
BEGIN
   IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'company_db') THEN
      PERFORM dblink_exec('dbname=postgres', 'CREATE DATABASE company_db');
   END IF;
END $$;
...
department_ranking AS (
    ...
    SELECT
        ec.*,
        d.department_name,
        RANK() OVER (PARTITION BY ec.department_id ORDER BY ec.total_compensation DESC NULLS LAST) AS comp_rank,
